...
export function CategoryLinksDialog({ open, onOpenChange, category }: CategoryLinksDialogProps) {
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const { error: uploadError } = await supabase.storage.from('icons').upload(`icons/${fileName}`, file, { cacheControl: '3600', upsert: false })
      if (uploadError) throw uploadError

      const { data: { publicUrl } } = supabase.storage.from('icons').getPublicUrl(`icons/${fileName}`)
                      ...
                      <div className="flex-1 min-w-0 flex flex-col">
                        <div className="flex items-center justify-between gap-2">
                          ...
                          <span
                            ...
                            className={`text-[10px] px-2 py-0.5 rounded flex-shrink-0 cursor-pointer select-none transition-colors border ${link.status === 'approved'
